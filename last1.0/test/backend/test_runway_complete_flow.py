...
async def create_runway_video(prompt_image: str, prompt_text: str, model: str = "gen4_turbo", **kwargs):
    """创建Runway图生视频任务"""
    print(f"\n{'='*80}")
    print(f"🎬 创建Runway图生视频任务")
    print(f"{'='*80}")
    print(f"模型: {model}")
    print(f"图片URL: {prompt_image}")
...
async def test_runway_models():
    """测试Runway模型"""
    print("="*80)
    print("🚀 Runway图生视频完整流程测试")
    print("="*80)
    ...
    test_cases = [
        {
            ...
            "promptText": "让logo动起来，添加动画效果",
            "model": "gen4_turbo",
            "duration": 5,
            "ratio": "1280:768",
            "watermark": False,
            "description": "Runway Gen4 Turbo - 图生视频"
        },
    ]
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        # 提取必需参数
        prompt_image = test_case.pop("promptImage")
        prompt_text = test_case.pop("promptText")
        model = test_case.pop("model", "gen4_turbo")

        # 步骤1: 创建任务
        task_id, create_result = await create_runway_video(
        ...
        if not task_id:
            ...
            results.append({
                "model": test_case.get("model", "gen4_turbo"),
...
if __name__ == "__main__":
    ...
    asyncio.run(test_runway_models())
