# -*- coding: utf-8 -*-
"""
测试Runway文生视频（Text-to-Video）完整流程
"""
...
async def create_runway_text2video(prompt_text: str, model: str = "gen4_turbo", **kwargs):
    """创建Runway文生视频任务"""
    print(f"\n{'='*80}")
    print(f"🎬 创建Runway文生视频任务")
    print(f"{'='*80}")
    print(f"模型: {model}")
    ...
    try:
        async with httpx.AsyncClient(timeout=30, trust_env=False) as client:
            response = await client.post(
                f"{BASE_URL}/runwayml/v1/text_to_video",
                headers={
            ...
            if response.status_code == 200:
                ...
                if "json" not in content_type.lower():
                    print(f"❌ 响应不是JSON格式，Content-Type: {content_type}")
...
async def query_runway_video_status(task_id: str):
    """查询Runway视频生成状态"""
    try:
        url = f"{BASE_URL}/runwayml/v1/tasks/{task_id}"
...
async def wait_for_runway_completion(task_id: str, max_wait_seconds: int = 600):
    """等待Runway视频生成完成"""
    print(f"\n{'='*80}")
    print(f"⏳ 等待视频生成完成...")
    print(f"{'='*80}")
    ...
    while True:
        ...
        result = await query_runway_video_status(task_id)
...
async def test_runway_text2video():
    """测试Runway文生视频"""
    print("="*80)
    print("🚀 Runway文生视频完整流程测试")
    print("="*80)

    # 测试用例
    test_cases = [
        {
            "promptText": "一只可爱的橘猫在阳光明媚的花园里追逐蝴蝶，画面温馨美好",
            "model": "gen4_turbo",
            "duration": 5,
            "ratio": "1280:768",
            "watermark": False,
            "description": "Runway Gen4 Turbo - 文生视频"
        },
    ]
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        # 提取必需参数
        prompt_text = test_case.pop("promptText")
        model = test_case.pop("model", "gen4_turbo")

        # 步骤1: 创建任务
        task_id, create_result = await create_runway_text2video(
        ...
        # 步骤2: 等待完成
        final_result = await wait_for_runway_completion(task_id, max_wait_seconds=300)
        ...
        if final_result:
            ...
            if code is not None:
                ...
                video_url = final_result.get("video") or final_result.get("video_url")
...
if __name__ == "__main__":
    ...
    asyncio.run(test_runway_text2video())
